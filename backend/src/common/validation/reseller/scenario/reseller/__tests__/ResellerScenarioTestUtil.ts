...
export class ResellerScenarioTestUtil {
  ...
  public async callAndCheckGetSupplierStep(): Promise<void> {
    const getSupplierResponse = await request(this.server).get('/reseller/octo/supplier').set(this.headers).send();
  ...
  public async callAndCheckGetProductsStep(): Promise<void> {
    const getProductsResponse = await request(this.server).get('/reseller/octo/products').set(this.headers).send();
  ...
  public async callAndCheckGetProductStep(): Promise<void> {
    const getProductResponse = await request(this.server)
      .get(`/reseller/octo/products/${this.getProductsData![0].id}`)
      .set(this.headers)
      .send();
    ...
    const validateQuestionAnswersResponse = await request(this.server)
      .post(
        `/reseller/session/${this.sessionId}/validate-question-answers/${this.scenario.getId()}/${StepId.GET_PRODUCT}`,
      )
      .set(this.headers)
      .send({
  ...
  public async callAndCheckGetAvailabilityCalendar(): Promise<void> {
    ...
    const getAvailabilityCalendarResponse = await request(this.server)
      .post('/reseller/octo/availability/calendar')
      .set(this.headers)
  ...
  public async callAndCheckGetAvailability(): Promise<void> {
    ...
    const getAvailabilityResponse = await request(this.server)
      .post('/reseller/octo/availability')
      .set(this.headers)
  ...
  public async callAndCheckBookingConfirmation(): Promise<void> {
    ...
    const bookingConfirmationResponse = await request(this.server)
      .post(`/reseller/octo/bookings/${this.bookingReservationData!.uuid}/confirm`)
      .set(this.headers)
      .send(bookingReservationPayload);
  ...
  public async callAndCheckBookingCancellation(): Promise<void> {
    ...
    const bookingCancellationResponse = await request(this.server)
      .post(`/reseller/octo/bookings/${this.bookingConfirmationData!.uuid}/cancel`)
      .set(this.headers)
  ...
  private async checkSession(
    ...
    const getSessionResponse = await request(this.server)
      .get(`/reseller/session/${this.sessionId}`)
      .set(this.headers)
      .send();
