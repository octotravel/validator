...
export class BookingEndpointValidator {
  ...
  public validate = (data: ValidateData): ValidatorError[] => {
    ...
    const errors = [
      ...
      booking?.product ? StringValidator.validate(`${this.path}.product.id`, booking?.product?.id, {
        equalsTo: productId,
      }) : null,
      StringValidator.validate(`${this.path}.optionId`, booking?.optionId, {
        equalsTo: optionId,
      }),
      booking?.option ? StringValidator.validate(`${this.path}.option.id`, booking?.option?.id, {
        equalsTo: optionId,
      }) : null,
