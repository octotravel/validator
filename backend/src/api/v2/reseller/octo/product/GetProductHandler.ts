...
import { GetProductSchema, getProductSchema } from './GetProductSchema';
...
export class GetProductHandler implements RequestHandler {
  ...
  public async handleRequest(request: IRequest): Promise<Response> {
    ...
    try {
      const validatedSchema = await SchemaValidator.validateSchema<GetProductSchema>(getProductSchema, requestPayload);
      const product = await this.productFacade.getProduct(validatedSchema.productId, sessionId);
