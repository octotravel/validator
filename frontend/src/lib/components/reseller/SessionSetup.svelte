<script lang="ts">
    ...
    const updateSession = () => {
        ...
        $resellerSessionStore.session.capabilities = selectedCapabilities.map((capability) => capability.id) ;
            ...
            <Stepper stepTerm='Session Setup' on:complete={updateSession}>
                <Step>
                    ...
                    <div class="w-96 mx-auto mt-5">
                        <input class="input" type="text" placeholder="Name" bind:value={$resellerSessionStore.session.name} />
                    </div>
                        ...
                        <div class="flex gap-4">
                            <ListBox multiple>
                                {#each $capabilitiesStore.capabilities.slice(0, 9) as capability}
                                    <ListBoxItem bind:group={selectedCapabilities} name={capability.id} value={capability} active='variant-ghost-secondary'>
